"""
LinkedIn extractor — PATCH 2 (multi-language + save-dialog handling).

Builds on PATCH 1 with:
- Multi-language button labels (Italian, Spanish, French, German, Portuguese, Dutch, English)
- "Save this application?" dialog auto-Discard handler
- Resume radio button auto-selection (keeps first/default if present)
- Stuck-state detection (clicks Discard, marks NEEDS_ANSWERS)
- Better progress logging (% step in modal)
"""
...
BTN_NEXT_LABELS = [
    ...
    "Weiter",
    # Portuguese
    "Próximo", "Próxima", "Continuar",
    # Dutch
    "Volgende",
]
BTN_REVIEW_LABELS = [
    "Review",
    "Rivedi", "Revisa", "Revoir", "Überprüfen",
    "Revisar", "Beoordelen",
]
BTN_SUBMIT_LABELS = [
    ...
    "Envoyer la candidature", "Envoyer",
    "Bewerbung absenden", "Absenden",
    "Verstuur sollicitatie", "Verstuur",
]
BTN_DISMISS_LABELS = ["Dismiss", "Chiudi", "Cerrar", "Fermer", "Schließen", "Sluiten"]
BTN_DISCARD_LABELS = [
    "Discard", "Scarta", "Descartar", "Ignorer",
    "Verwerfen", "Verwijderen", "Annulla", "Cancel",
]


def _xpath_button_any(labels: list[str]) -> str:
    """Build XPath that matches buttons containing any of the given label texts."""
    conditions = []
    for lbl in labels:
        # match either button text or descendant span text
        conditions.append(f"normalize-space(.)='{lbl}'")
        conditions.append(f".//span[normalize-space(.)='{lbl}']")
        conditions.append(f"contains(., '{lbl}')")
    return "//button[" + " or ".join(conditions) + "]"
...
SELECTORS = {
    ...
    "btn_discard": (By.XPATH, _xpath_button_any(BTN_DISCARD_LABELS)),

    # Save-this-application confirmation dialog
...
DIVERSITY_KEYWORDS = [
    "gender identity", "gender", "disability", "veteran",
    "race", "ethnicity", "ethnic", "sexual orientation",
    "transgender", "lgbtq", "pronoun", "self-identify", "self identify",
    # Italian
...
DECLINE_OPTION_PATTERNS = [
    "prefer not", "decline", "don't wish", "do not wish",
    "not to say", "self-identify", "self identify", "rather not",
    "i don't want", "not specified",
    # Italian
...
class LinkedInExtractor(BaseExtractor):
    ...
    def login(self, email, password, totp_secret=""):
        ...
        try:
            ...
            logger.warning("Login form not visible — maybe already logged in.")
        end = time.time() + 120
        while time.time() < end:
            url = d.current_url
    ...
    # ------------------------------------------------------------------
    # APPLY — PATCH 2: stuck detection, save dialog handler, progress log
    # ------------------------------------------------------------------
    def apply(self, job, resume_path, mode="semi_auto"):
        ...
        for step in range(max_steps):
            ...
            # Stuck detection: same progress 3 times in a row
            ...
            if stuck_count >= 2:
                logger.warning(f"🚧 Stuck at {progress}% for {stuck_count} iterations. Aborting.")
                ...
                return ApplicationResult(
                    ...
                    error_message=f"Stuck at {progress}%. Form may need manual answers.",
                    qa_log=qa_log, unanswered_questions=unanswered)
            ...
            # Try buttons in order: Submit → Review → Next
            if self._click_if_present(SELECTORS["btn_submit"]):
                if mode == "safe_auto":
                    logger.warning("⏸️  SAFE MODE — press ENTER to confirm.")
            ...
            if self._click_if_present(SELECTORS["btn_next"]):
                human_sleep(1.5, 2.5)
                # If unanswered AND we're in semi_auto, stop here cleanly
            ...
            logger.warning("No Next/Review/Submit button found.")
            self._screenshot_for_debug(job.job_id, "no_button")
            self._close_and_discard()
            return ApplicationResult(
                status=ApplyStatus.FAILED,
                error_message="No navigation button on modal step.",
                qa_log=qa_log, unanswered_questions=unanswered)
    ...
    # ------------------------------------------------------------------
    # NEW: ensure a resume is selected
    # ------------------------------------------------------------------
    def _ensure_resume_selected(self, modal):
        """If resume list shown with radio buttons, select first one if none selected."""
        try:
            radios = modal.find_elements(
                ...
                ".ember-view input[type='radio']"
            )
            if not radios:
                return
            # If any already selected, leave alone
            if any(r.is_selected() for r in radios):
                return
            # Click first
            try:
                radios[0].click()
                logger.info("📄 Auto-selected first resume option.")
                human_sleep(0.5, 1.0)
            except ElementClickInterceptedException:
    ...
    # ------------------------------------------------------------------
    # NEW: close modal AND handle "Save this application?" dialog
    # ------------------------------------------------------------------
    def _close_and_discard(self):
        """Dismiss modal, then if Save dialog appears, click Discard."""
        ...
        # Step 2: Wait briefly for save dialog, then click Discard if it appears
        end = time.time() + 3
        while time.time() < end:
            try:
                self.driver.find_element(*SELECTORS["save_dialog"])
                # Dialog present — find Discard button
                self._click_discard_with_retry()
                logger.info("🗑️  Discarded draft application.")
                human_sleep(0.5, 1.2)
                return
            except NoSuchElementException:
                time.sleep(0.3)
        # No save dialog appeared — that's fine

    def _click_discard_with_retry(self):
        """Try multiple strategies to click Discard button in save dialog."""
        # Try multi-lang Discard XPath
        try:
            btns = self.driver.find_elements(*SELECTORS["save_dialog_discard"])
            for btn in btns:
            ...
            pass
        # Fallback: any button containing Discard text in any nearby dialog
        try:
        ...
        for r in modal.find_elements(By.CSS_SELECTOR, "input[type='radio']"):
            try:
                ...
                if not name or name in seen:
                    continue
                # Skip resume-selection radios (handled by _ensure_resume_selected)
            ...
            try:
                ...
                if any(kw in label for kw in
                       ["terms", "privacy", "consent", "agree", "acknowledge",
                        "i confirm", "i certify",
                        # Italian
                        "termini", "privacy", "consenso", "accetto",
                        # Spanish
                        "términos", "acepto",
                        # French
    ...
    def _lookup_answer(self, question):
        if not question:
            return None
    ...
    def _verify_submitted(self):
        success_indicators = [
            (By.XPATH, "//h2[contains(.,'Application sent') or contains(.,'application was sent') or contains(.,'Candidatura inviata') or contains(.,'Solicitud enviada') or contains(.,'Candidature envoyée')]"),
        ...
        while time.time() < end:
            for by, sel in success_indicators:
                try:
                    ...
                    if el.is_displayed():
                        logger.info(f"✅ Submit confirmed: {sel}")
                        self._close_and_discard()
                        return True
                except NoSuchElementException:
                    continue

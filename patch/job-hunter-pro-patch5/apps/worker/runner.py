"""
Main bot orchestrator — PATCH 5.

Changes vs PATCH 3:
- Heartbeat sent every iteration
- atexit handler clears state files on exit (clean OR crash)
- AI connection tested at startup before bot starts
"""
...
def _heartbeat_thread(stop_event: threading.Event):
    """Background thread that keeps heartbeat fresh during long Selenium ops."""
...
def run_bot(config_path: str = "config.yaml"):
    ...
    # Clear stale state from previous crash
    controller.reset()
    controller.set_state("running")
    controller.beat()

    # Register cleanup handler — runs even on crash/exit
    def _cleanup():
    ...
    try:
        for platform_name, pcfg in config["platforms"].items():
            ...
            if platform_name not in EXTRACTOR_REGISTRY:
                logger.warning(f"No extractor for {platform_name}.")
                continue
        ...
        try:
            ...
            pass
        # atexit handler clears state files
